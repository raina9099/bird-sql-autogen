...
def package_sqls(sql_path, db_root_path, mode='gpt', data_mode='dev'):
    ...
    if mode == 'gpt':
        ...
        for idx, sql_str in sql_data.items():
            if type(sql_str) == str and type(idx) == str:
                # sql, db_name = sql_str.split('\t----- bird -----\t') #sql = sql string, db_name = california_schools
                sql = sql_str
                db_name = idx.split('/')[0]
            else:
...
def compute_acc_by_diff(exec_results, diff_json_path):
    # print(f"exec_results\n{exec_results}")
    # print(f"diff_json_path\n{diff_json_path}")
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--diff_json_path',type=str,default='./data/mini_dev_data/dev.json')
    ...
    # TODO: Remove
    # gt_queries, db_paths_gt = [gt_queries[0]], [db_paths_gt[0]]
    gt_queries, db_paths_gt = gt_queries[:NUM_SAMPLES], db_paths_gt[:NUM_SAMPLES]
    # print(f"pred_queries:\n{pred_queries}")
    # print(f"gt_queries:\n{gt_queries[0]}")
